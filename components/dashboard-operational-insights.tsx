...
export function DashboardOperationalInsights({ stats, trends, loading, className }: DashboardOperationalInsightsProps) {
  ...
  if (loading && !stats) {
    return (
      <div className={cn("grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5", className)}>
        {[0, 1, 2, 3, 4, 5].map((i) => (
          <Skeleton key={i} className="ws-shimmer h-40 min-h-[10rem] rounded-[1.2rem] border border-white/40 sm:h-44" />
        ))}
      </div>
    );
  }
  ...
  return (
    <div className={cn("ws-insights-bento", className)}>
      <InsightPanel
        title={uiLang === "hi" ? "कन्वर्ज़न फनल" : "Conversion funnel"}
        icon={Percent}
        iconTone="emerald"
        spanClass="ws-insights-span-7"
      >
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="ws-conversion-metric">
            ...
            <p className="ws-conversion-metric-value text-sky-900 dark:text-sky-100">
              {model.conversion.leadToProposalDisplay}
            </p>
          ...
          <div className="ws-conversion-metric">
            ...
            <p className="ws-conversion-metric-value text-emerald-900 dark:text-emerald-100">
              {model.conversion.proposalToOrderDisplay}
            </p>
            <div className="ws-conversion-bar">
              <div
        ...
        <p className="mt-2 text-[10px] font-medium leading-snug text-slate-400 dark:text-[#6E7681]">
      ...
      <InsightPanel
        ...
        icon={AlertTriangle}
        iconTone="warning"
        spanClass="ws-insights-span-5"
      >
        <ul className="max-h-[min(20rem,50vh)] space-y-2.5 overflow-y-auto pr-0.5 sm:max-h-none">
      ...
      <InsightPanel
        ...
        icon={ClipboardCheck}
        iconTone="sky"
        spanClass="ws-insights-span-6"
      >
        {model.followUps.length === 0 ? (
          <p className="text-sm leading-relaxed text-slate-600 dark:text-[#8B949E]">
        ...
        <div className="space-y-2">
          <div className={trendRowClass(model.collectionTrend.revenueTrend, "revenue")}>
            ...
            <p className="mt-1 text-xs font-semibold leading-snug text-slate-800 dark:text-white">
              {model.collectionTrend.revenueTrend ?? (uiLang === "hi" ? "अगली सिंक के बाद" : "After next sync")}
            </p>
          </div>
          <div className={trendRowClass(model.collectionTrend.pendingTrend, "pending")}>
            ...
            <p className="mt-1 text-xs font-semibold leading-snug text-slate-800 dark:text-white">
              {model.collectionTrend.pendingTrend ?? (uiLang === "hi" ? "अगली सिंक के बाद" : "After next sync")}
            </p>
          </div>
        </div>
        <p
      ...
      >
        ...
        {model.overdue.items.length === 0 ? (
          <p className="rounded-xl border border-emerald-200/80 bg-emerald-50/50 px-3 py-2.5 text-sm leading-relaxed text-emerald-900 dark:border-emerald-500/25 dark:bg-emerald-950/20 dark:text-emerald-200">
            ...
            {model.recentActivity.map((row) => (
              <li key={row.id}>
                ...
                >
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-semibold text-slate-800 dark:text-white">{row.name}</p>
                    <p className="mt-0.5 line-clamp-2 text-xs text-slate-500 sm:line-clamp-1">{row.detail}</p>
                  </div>
      ...
      <InsightPanel
        ...
        icon={Lightbulb}
        iconTone="indigo"
        spanClass="ws-insights-span-full"
      >
