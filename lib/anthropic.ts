...
13) CRITICAL — mp_govt_subsidy_amount_inr: extract from the line labelled "M.P. Govt. Subsidy Amount" / "MP Govt Subsidy" / "Subsidy Amount" / "AGJY" / "Atal Griha Jyoti" / "Indira Griha Jyoti" / "Mukhyamantri … Subsidy". MPEZ/MPCZ/MPWZ bills print this as a NEGATIVE rupee value (examples: "-544.96", "-100.00", "( 100.00 )"). PRESERVE THE SIGN — return a NEGATIVE NUMBER. If the line is printed as "0.00" (consumer > 150 u in FY 25-26, no subsidy), return 0 (not null). Only return null when the subsidy line is entirely absent from the bill. Do NOT confuse this with "Rebate & Incentive", "Online / Advance Payment Incentive" or "Interest On Security Deposit" — those are SEPARATE negative lines and must NOT be combined into mp_govt_subsidy_amount_inr. **Our tariff engine never uses this field to compute subsidy** — subsidy = rules + units only; this extraction is for column hygiene and optional audit compare.
14) CRITICAL — tod_rebate_inr: extract ONLY from lines labelled "TOD" / "Time of Day" / "ToD Rebate & Surcharge" / "Other / TOD…". Signed ₹ as printed (rebate often negative). NEVER put this amount into mp_govt_subsidy_amount_inr and never merge the two fields.
